...
class DeepFakeDetector(nn.Module):
    def __init__(self, num_classes=2, latent_dim=2048, lstm_layers=1, hidden_dim=512, bidirectional_lstm=False,
                 dropout_rate=0.5):
        super(DeepFakeDetector, self).__init__()
        cnn_model = models.resnext50_32x4d(weights=None)
        self.cnn_features = nn.Sequential(*list(cnn_model.children())[:-2])
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.lstm = nn.LSTM(input_size=latent_dim, hidden_size=hidden_dim, num_layers=lstm_layers,
                            bidirectional=bidirectional_lstm, batch_first=True)
